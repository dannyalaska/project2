...
module SoundBankModule
    ...
    def find_sound sound
      all_sounds = HTTParty.get('http://www.freesound.org/apiv2/search/text/?query="kick" -double&token=65c0592c57ffd37ddcbdec010b21f19d51727d81')["results"]
      if all_sounds.length > 0
        all_sounds.map { |sound| get_individual_sounds(sound["id"])["previews"]["preview-lq-mp3"]}
      end
    end
    puts find_sound("kick")
 HTTParty.get('http://www.freesound.org/apiv2/search/text/?query=kick&fields=name,preview-lq-mp3
